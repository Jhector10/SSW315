...
private boolean search(BinaryNode<AnyType> curr, AnyType target) {
	if (curr == null) {
	 ...
	 if (target.compareTo(curr.element) < 0) {
		 return search(curr.left, target);
	 } else if(target.compareTo(curr.element) > 0) {
...
private void traverseInOrder(BinaryNode<AnyType> curr) {
	if( curr.left != null ) 
		traverseInOrder(curr.left);            // Left
	System.out.println( curr.element );       // Node
	if( curr.right != null )
		traverseInOrder(curr.right);           // Right
...
private void traversePreOrder(BinaryNode<AnyType> curr, int indent) {
	...
	StringBuilder indentTree = new StringBuilder();
	
	for (int i = 0; i < indent; i++) { 
		// for each line, indent the amount times of indent
		indentTree.append("    ");
	}
	
	if (curr == null) 
		System.out.println(indentTree.toString() + ".");
	
	else {
		System.out.println( indentTree.toString() + curr.element );
